...
function getShardNum(target, record) {
  const personId = record.data.payload.personId.replace(/\W/, '')
  return parseInt(personId, 36) % target.shardCount
}

function queueUrl(target, record) {
  // TODO- get appropriate target from record
  const shardNum = getShardNum(target, record)
  console.log(`Publishing to shardNum ${shardNum}`)
  // TO DO- make FIFO optional
...
function send(service, target, records, callback) {
