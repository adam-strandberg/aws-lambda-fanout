...
exports.send = function(service, target, records, callback) {
  switch(target.collapse) {
    case "JSON": {
      ...
      service.sendMessage({ QueueUrl: target.destination, MessageBody: JSON.stringify(entries), MessageGroupId: "fakeId", MessageDeduplicationId: Math.random().toString(36).substring(0, 13) }, callback);
      break;
    }
    case "concat-b64": {
      ...
      service.sendMessage({ QueueUrl: target.destination, MessageBody: JSON.stringify(entries).toString('base64'), MessageGroupId: "fakeId", MessageDeduplicationId: Math.random().toString(36).substring(0, 13) }, callback);
      break;
    }
    case "concat": {
      // We have multiple messages, collapse them in a single buffer
      var data = Buffer.concat([].concat.apply([], records.map(function(record) { return record.data; })));
      service.sendMessage({ QueueUrl: target.destination, MessageBody: JSON.stringify(entries).toString(), MessageGroupId: "fakeId", MessageDeduplicationId: Math.random().toString(36).substring(0, 13) }, callback);
      break;
    }
    default: {
      // We have a single message, let's send it
      service.sendMessage({ QueueUrl: target.destination, MessageBody: records[0].data.toString(), MessageGroupId: "fakeId", MessageDeduplicationId: Math.random().toString(36).substring(0, 13) }, callback);
    }
  }
};
