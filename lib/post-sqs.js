...
function send(service, targets, records, callback) {
  ...
  if (!target) {
    ...
    console.log(`Processed record with eventType ${JSON.parse(record.data.toString()).eventType} with no corresponding target`)
  } else {
    switch(target.collapse) {
      ...
      default: {
        ...
        console.log(`calling sendMessage for ${exports.queueUrl(target, record)}`)
