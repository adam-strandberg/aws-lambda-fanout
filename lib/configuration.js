...
var sqsFifoConfigTarget = {
  sourceArn: null,
  id: null,
  type: null,
  destinationBaseQueueName: null,
  shardCount: 1,
  role: null,
  region: null,
  externalId: null,
...
function readTarget(record, serviceDefinitions) {
  var target = DDB.parseDynamoDBObject(record, sqsFifoConfigTarget);
  ...
  if((typeof target.active) != "string") {
    console.error("Invalid configuration for target '" + target.id + "' of type '" + target.type + "', 'active' property must be a string. Ignoring target");
    return null;
  }
...
function get(serviceDefinitions, callback) {

  if(mappings.nextConfigurationRefresh > Date.now()) {
    // No need to refresh, just return OK
    callback(null, mappings.targets);
    return;
  }
  ...
  var runQuery = function(exclusiveStartKey) {
    var params = { TableName: config.configurationTable, Select: "ALL_ATTRIBUTES", KeyConditionExpression: "active = :active", ExpressionAttributeValues: { ":active": { S: "true" } }};
    ...
    dynamo.query(params, function(err, data) {
      ...
      if(config.debug) {
        console.log("Loaded configuration table ", config.configurationTable, "for function", process.env.AWS_LAMBDA_FUNCTION_NAME, "found", entries.length, "targets");
        entries.forEach(function(target) {
          console.log(" - Id:", target.id, "of type", target.type);
        });
      }
